package com.hundsun.zjfae.common.utils.stepcounter.service;
...
import com.hundsun.zjfae.common.utils.stepcounter.constant.Constant;
...
public class StepService extends Service implements SensorEventListener {
    ...
    private String notificationId = "StepServiceId";
    private String notificationName = "StepService";
...
//        builder = new Notification.Builder(this.getApplicationContext());
         ...
         * 总之有需要的可以和我交流
         */
//        nfIntent = new Intent(this, HomeActivity.class);
...
//        notificationManager.notify(110, stepNotification);
        ...
        startForeground(110, getNotification());// 开始前台服务
    ...
    private Notification getNotification() {
        Notification.Builder builder = new Notification.Builder(this)
                .setSmallIcon(R.drawable.zj_logo)
                .setContentTitle("浙金中心")
    ...
    private void initTodayData() {
        ...
        if (PreferencesHelper.getStepToday(getApplicationContext()).equals("")) {
            PreferencesHelper.setStepToday(getApplicationContext(), CURRENT_DATE);//保存当前的日子
        }
        if (PreferencesHelper.getStepTodayCurrentTime(getApplicationContext()).equals("") || PreferencesHelper.getStepTodayCurrentTime(getApplicationContext()).equals(new Date().getTime() + "")) {
            //如果之前的日子数据为空 说明首次安装  如果跟现在的时间不一样 那不能覆盖之前的 必须先处理之前的步数
            PreferencesHelper.setStepTodayCurrentTime(getApplicationContext(), new Date().getTime() + "");
        }
    }
    ...
    public void onSensorChanged(SensorEvent event) {
        if (stepSensor == 0) {
            ...
            Log.e("StepService", "当前计步器数量----" + tempStep);
            if (!hasRecord) {
                Log.e("StepService", "当天没有历史起始步数记录----");
                //当天没有历史起始步数记录
                int before = (int) PreferencesHelper.getLastSensorStep(getApplicationContext());
                Log.e("StepService", "之前保存的最后的计步器数量----" + before);
                if (PreferencesHelper.getStepToday(getApplicationContext()).equals(CURRENT_DATE)) {
                    Log.e("StepService", "同一天");
                    //如果是同一天的话
                    if (before == 0) {//说明是首次安装
                        Log.e("StepService", "首次安装");
                        PreferencesHelper.setLastSensorStep(getApplicationContext(), tempStep);
                        PreferencesHelper.setStepTodayCurrentTime(getApplicationContext(), new Date().getTime() + "");
                    } else {
                        if (tempStep >= before) {
                    ...
                    Log.e("StepService", "上次保存的时间----" + PreferencesHelper.getStepTodayCurrentTime(getApplicationContext()));
                    Log.e("StepService", "现在保存的时间----" + new Date().getTime() + "");
                    if (TimeUtil.daysBetween2(PreferencesHelper.getStepTodayCurrentTime(getApplicationContext()), new Date().getTime() + "") > 24) {
                        //如果超过24小时了 就废弃不用
                        Log.e("StepService", "大于24小时");
                        PreferencesHelper.setStepTodayCurrentTime(getApplicationContext(), new Date().getTime() + "");
                    } else if (TimeUtil.daysBetween2(PreferencesHelper.getStepTodayCurrentTime(getApplicationContext()), new Date().getTime() + "") > 0) {
                        //小于24小时 保存为昨天的数据
