...
public class FingerprintUtil {
    ...
    public static void callFingerPrint(Context context, final OnCallBackListenr listener) {
        FingerprintManagerCompat managerCompat = FingerprintManagerCompat
                .from(context);
        ...
        cancellationSignal = new CancellationSignal(); // 必须重新实例化，否则cancel
        // 过一次就不能再使用了
    ...
    public static boolean callFingerPrint(Context context){
        FingerprintManagerCompat managerCompat = FingerprintManagerCompat
                .from(context);
