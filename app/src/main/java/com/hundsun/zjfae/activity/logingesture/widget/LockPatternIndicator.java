...
public class LockPatternIndicator extends View {
	 ...
	 * @return Cell
	 */
	private IndicatorCell getCellBetweenTwoCells(LockPatternView.Cell preCell, LockPatternView.Cell nextCell) {
		//two cells are in the same row
		if (preCell.getRow() == nextCell.getRow()) {
			if (Math.abs(nextCell.getColumn() - preCell.getColumn()) > 1) {
				return mIndicatorCells[preCell.getRow()][1];
			} else {
				return null;
			}
		}
		//two cells are in the same column
			...
			if (Math.abs(nextCell.getRow() - preCell.getRow()) > 1) {
				return mIndicatorCells[1][preCell.getColumn()];
			} else {
				return null;
			}
		}
	...
	private void drawToCanvas(Canvas canvas) {
		...
		//LockPatternView.Cell tempCell  = new LockPatternView.Cell();
		for(int i = 0; i < mIndicatorCells.length; i++) {
			for(int j = 0; j < mIndicatorCells[i].length; j++) {
