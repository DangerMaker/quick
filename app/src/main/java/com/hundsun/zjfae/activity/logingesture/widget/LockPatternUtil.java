...
public class LockPatternUtil {
     ...
     * @return
     */
    public static byte[] patternStringToHash(String data) {
    ...
    public static boolean checkPattern(List<LockPatternView.Cell> pattern) {
        String fingerprintPassWord = UserInfoSharePre.getGessturePwd();
        if (pattern == null || fingerprintPassWord == null) {
            return false;
        } else {
            byte[] bytes2 = patternToHash(pattern);
            String passWord = new String(bytes2);
            return fingerprintPassWord.equals(passWord);
        }
    }

    public static boolean checkPatternBlockchain(List<LockPatternView.Cell> pattern){

        String blockchainPassWord = UserInfoSharePre.getBlockchainGessturePwd();

        if (pattern == null || blockchainPassWord.equals("")){
            ...
            byte[] bytes2 = patternToHash(pattern);
            String passWord = new String(bytes2);
            return blockchainPassWord.equals(passWord);
