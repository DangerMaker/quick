 ...
 * @Description:短期预约列表
 * @Author: zhoujianyu
 * @Time: 2018/9/17 13:50
 */
public class ShortReserveListFragment extends BaseFragment implements OnRefreshListener, OnLoadMoreListener, ReserveListView {
    ...
    protected int getLayoutId() {
        return R.layout.fragment_short_reserve_list;
    }
...
//    }
    ...
    private void onRefreshData() {
        ...
        page = 1;
        mPresenter.getReserveListData(page, "02");
    }
    ...
    public void loadData(List<ProductOrderInfoPB.PBIFE_trade_queryProductOrderInfo.TaProductOrderInfoWrapList> list) {
        if (page == 1) {
            if (list == null || list.size() == 0) {
                ...
                mList.clear();
                mList = list;
                adapter = new LongReserveAdapter(getActivity(), mList, "02", new LongReserveAdapter.onItemClicker() {
                    @Override
                    public void OnItemClick(String orderProductCode) {

                         ShortReserveListFragment.this.orderProductCode = orderProductCode;
